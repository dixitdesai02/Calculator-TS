...
const handleClick = (key: string) => {
    if (isNumber(key) || isOperator(key) || isKeyword(key)) {
        ...
        switch (key) {
            ...
            case "|x|":
                input.value = input.value && Math.abs(eval(input.value)).toString();    
                break;
            ...
            case "3√x":
                input.value = Math.cbrt(+input.value).toString();
                break;
            ...
            case "M+":
                ...
                var b = +input.value;
                var c = a+b;
                localStorage.setItem("memory", c.toString());
...
const factorial = (num: string): number => {
...
const validSymbol = (value: string): string => {
    if (value === "×")   
        return "*";
    else if (value === "÷") {
        return "/";
...
const formatExpression = (expression: string): string => {
    ...
    if ((second as HTMLElement).innerText === "2nd" || (second as HTMLElement).innerText === "nd") {
        // Sin
...
const calculateLog = (expression: string, isLog: boolean): string => {
    let expr;
    if (isLog)
        expr = "log";
    else
    ...
    if (substr.includes("π"))
        substr = substr.replace("π", Math.PI.toString());
    if (substr.includes("e"))
        substr = substr.replace("e", Math.E.toString());
...
const unitConversion = (value: string): string => {
    const unitName = (unit as HTMLElement).innerText;

    if (unitName === "DEG") {
        (unit as HTMLElement).innerText = "RAD";
        if (value !== "")
    ...
    (unit as HTMLElement).innerText = "DEG";
    if (value !== "")
...
const changeButtons = (): void => {
    ...
    if ((second as HTMLElement).innerText === "2nd" || (second as HTMLElement).innerText === "nd") {
        inv.forEach((i)=>{
            (<HTMLElement>i).style.display = "inline-block";
        });
        ...
        inv.forEach((i)=>{
            (<HTMLElement>i).style.display = "none";
        });
